...
final class FakeEventStore: EventStore {
    ...
    init(stored: [StoredEvent], readAccess: Bool, writeAccess: Bool) {
    ...
    func insert(_ record: CalendarEvent) async throws {
        state.withLock { current in
            ...
            current.nextId += 1
            current.stored.append(StoredEvent(identifier: identifier, record: record))
        }
    }
    ...
    func update(identifier: String, with record: CalendarEvent) async throws {
        state.withLock { current in
            if let index = current.stored.firstIndex(where: { $0.identifier == identifier }) {
                current.stored[index] = StoredEvent(identifier: identifier, record: record)
            }
        }
    }
}
