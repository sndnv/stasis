...
@click.command(name='remove', short_help='Remove inactive operations.')
@click.argument('operation', type=click.UUID)
@click.pass_context
def remove_operation(ctx, operation):
    """Remove an inactive OPERATION."""
    click.confirm(
        'Operation [{}] will be removed. Do you want to continue?'.format(operation),
        abort=True
    )

    response = ctx.obj.api.operation_remove(operation)
...
cli.add_command(remove_operation)
