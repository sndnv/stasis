package stasis.test.specs.unit.persistence.backends.file.container.ops
...
import stasis.persistence.backends.file.container.ops.{AutoCloseSupport, ContainerLogOps, ConversionOps}
import stasis.persistence.backends.file.container.Container
import stasis.test.specs.unit.AsyncUnitSpec
import stasis.test.specs.unit.persistence.backends.file.container.TestOps
        ...
        .recover {
          case NonFatal(e) =>
            e.getMessage should be(
              s"Failed to create container log [$containerLogPath]: [java.nio.file.FileAlreadyExistsException: $containerLogPath]"
            )
        }
    }
  }
  ...
  it should "fail to destroy a nonexistent container log" in {
    ...
    ContainerLogOps
      ...
      .recover {
        case NonFatal(e) =>
          e.getMessage should be(
            s"Failed to destroy container log [$containerLogPath]: [java.nio.file.NoSuchFileException: $containerLogPath]"
          )
      }
  }
  ...
  it should "fail to add entries to an invalid container log" in {
    ...
    ContainerLogOps
      ...
      .recover {
        case NonFatal(e) =>
          e.getMessage should be(
            s"Failed to update container log [$containerLogPath] with event [Add]: [java.nio.file.NoSuchFileException: $containerLogPath]"
          )
      }
  }
  ...
  it should "fail to load a list of crates from an invalid container log" in {
    ...
    ContainerLogOps
      ...
      .recover {
        case NonFatal(e) =>
          e.getMessage should be(
            s"Failed to retrieve crates from container log [$containerLogPath]: [java.nio.file.NoSuchFileException: $containerLogPath]"
          )
      }
  }
}
