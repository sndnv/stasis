...
class ClientPushDeviceSecretForm extends StatefulWidget {
  const ClientPushDeviceSecretForm({
    super.key,
    this.title,
  ...
  @override
  State createState() {
    return _ClientPushDeviceSecretFormState();
  }
}

class _ClientPushDeviceSecretFormState extends State<ClientPushDeviceSecretForm> {
  ...
  final _remotePasswordConfirmationController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    ...
    final remotePasswordField = TextFormField(
      ...
      decoration: const InputDecoration(
        ...
        prefixIcon: Tooltip(
          message:
              'Password override if the remote secret should have a different password. '
              'Your password is never stored, sent to the server or shared with other services.',
          child: Icon(Icons.help_center),
        ),
      ...
      autovalidateMode: AutovalidateMode.onUserInteraction,
      validator: (value) {
        if (value != _remotePasswordConfirmationController.text) {
    ...
    final remotePasswordConfirmationField = TextFormField(
      enabled: !_processingResponse,
      decoration: const InputDecoration(
        border: OutlineInputBorder(),
        prefixIcon: Tooltip(
        ...
        labelText: 'Confirm Remote Password',
      ),
      obscureText: true,
      enableSuggestions: false,
      autocorrect: false,
      controller: _remotePasswordConfirmationController,
      autovalidateMode: AutovalidateMode.onUserInteraction,
      validator: (value) {
        if (value != _remotePasswordController.text) {
          return 'Passwords do not match';
        } else {
          return null;
        }
    ...
    Widget pushButton = FilledButton(
    ...
    if (_showExtra) {
      extra = [
        ...
        Padding(padding: const EdgeInsets.all(8.0), child: remotePasswordConfirmationField),
  ...
  void dispose() {
    ...
    _remotePasswordConfirmationController.dispose();
