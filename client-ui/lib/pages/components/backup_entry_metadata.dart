...
class _BackupEntryMetadataState extends State<BackupEntryMetadata> {
  ...
  Widget build(BuildContext context) {
    return buildPage<DatasetMetadata>(
      of: () => widget.client.getDatasetMetadata(entry: widget.entry.id),
      builder: (context, metadata) {
        final theme = Theme.of(context);

        final originalEntities = metadata.filesystem.entities.keys.toList()..sort();

        final entities = _showChangedFilesOnly
            ? originalEntities.where((e) {
                final entityState = metadata.filesystem.entities[e];
                final entityMetadata = (metadata.contentChanged[e] ?? metadata.metadataChanged[e]);

                final hasChanged = entityState?.entityState == 'new' || entityState?.entityState == 'updated';
                final isFile = entityMetadata is FileEntityMetadata;
        ...
        final summary = ListView(
          ...
          children: [DatasetEntrySummary.build(context, entry: widget.entry)],
        );
        ...
        final showAsTree = FloatingActionButton.small(
          ...
          onPressed: () {
            ...
            final tree = FileTree.fromPaths(
              ...
              pathStyle: (path) {
                final state = metadata.filesystem.entities[path];
                    ...
                    return SimpleDialog(
                      children: [
                        DatasetMetadataEntitySummary.build(
                          ...
                          entity: path,
                          state: metadata.filesystem.entities[path]!,
                          metadataChanged: metadata.metadataChanged[path],
                          contentChanged: metadata.contentChanged[path],
                        ),
                      ],
                    );
                  },
        ...
        final changesList = ListView(
          ...
          children: (entities.toList()..sort()).map((e) {
            return DatasetMetadataEntitySummary.build(
              ...
              parentEntry: widget.entry,
              entity: e,
              state: metadata.filesystem.entities[e]!,
              metadataChanged: metadata.metadataChanged[e],
              contentChanged: metadata.contentChanged[e],
            );
          }).toList(),
        );
