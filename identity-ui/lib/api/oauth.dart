...
class JwtClaims {
  JwtClaims({required this.claims});

  final Map<String, dynamic> claims;

  String? get subject => claims['sub'];

  String? get audience => claims['aud'];

  static JwtClaims fromCredentials(oauth2.Credentials credentials) {
    final decodedClaims = utf8.decode(base64.decode(base64.normalize(credentials.accessToken.split('.')[1])));
    return JwtClaims(claims: json.decode(decodedClaims));
  }
}

class Keys {
  static const String _codeVerifier = 'stasis.identity_ui.oauth.code_verifier';
  static const String _state = 'stasis.identity_ui.oauth.state';
  static const String _credentials = 'stasis.identity_ui.oauth.credentials';
}
