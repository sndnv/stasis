  ...
  it should "support loading rules from a file" in {
    val expectedRules = Seq(
      Rule(
        ...
        directory = "/home/stasis",
        pattern = "**",
        comment = Some("include all files for the current user"),
        original = Rule.Original(
          line = "+ /home/stasis           **                  # include all files for the current user",
          lineNumber = 0
        )
      ),
      Rule(
        operation = Rule.Operation.Exclude,
        directory = "/home/stasis/.m2",
        pattern = "repository/**",
        comment = Some("exclude maven artifacts"),
        original = Rule.Original(
          line = "- /home/stasis/.m2       repository/**       # exclude maven artifacts",
          lineNumber = 1
        )
      ),
      Rule(
        operation = Rule.Operation.Exclude,
        directory = "/home/stasis/.ivy2",
        pattern = "{cache|local}/**",
        comment = Some("exclude ivy artifacts"),
        original = Rule.Original(
          line = "- /home/stasis/.ivy2     {cache|local}/**    # exclude ivy artifacts",
          lineNumber = 2
        )
      ),
      Rule(
      ...
      Rule(
        ...
        comment = None,
        original = Rule.Original(
          line = "- /home/stasis           **/*.{tmp|temp|part|bak|~}",
          lineNumber = 9
        )
      )
    )
